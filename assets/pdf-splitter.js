...
const BACKEND_ANALYZER_DEFAULT_URL = "http://127.0.0.1:8787";
...
function isLikelyLoopbackUrl(url) {
...
function getRuntimeAnalyzerConfig() {
  ...
  if (!analyzerUrl && isLocalHost) {
    analyzerUrl = BACKEND_ANALYZER_DEFAULT_URL;
  }
  analyzerUrl = analyzerUrl.replace(/\/+$/, "");

  const forceOn = queryUseAnalyzer === "1" || queryUseAnalyzer === "true";
  ...
  } else if (typeof appConfig.useBackendAnalyzer === "boolean") {
    useAnalyzer = appConfig.useBackendAnalyzer && Boolean(analyzerUrl);
  } else if (isLocalHost) {
    useAnalyzer = Boolean(analyzerUrl);
  } else {
    // In production, never call loopback URL.
    useAnalyzer = Boolean(analyzerUrl) && !isLikelyLoopbackUrl(analyzerUrl);
  }
...
const DOC_RULES = [
  {
    type: "INV",
    patterns: [/\binvoice\b/i, /commercial invoice/i, /tax invoice/i, /\binv\b/i]
  },
  { type: "PKL", patterns: [/packing list/i, /\bpkl\b/i] },
  {
    type: "BOL",
    patterns: [/bill of lading/i, /\bb\/l\b/i, /\bb\/l no\b/i, /\bbl no\b/i]
  },
...
function readBackendConfidence(data) {
  ...
  return clampConfidence(
    data.confidence ?? data.score ?? data.result?.confidence ?? data.prediction?.confidence ?? 0
  );
}
...
function detectDocType(text, headerHint = "") {
  ...
  // Explicit conflict resolver for INV/PKL/BOL pages.
  if (
    invScore >= 6 &&
    invScore >= pklScore &&
    invScore >= bolScore &&
  ...
  if (hcScore >= 5 && hcScore >= pcScore && hcScore >= cooScore && hcScore >= bolScore) {
    return "HC";
  }
  if (cooScore >= 5 && cooScore >= pcScore && cooScore >= hcScore && cooScore >= bolScore) {
    return "COO";
  }
  if (bolScore >= 8 && bolScore >= pklScore + 2) {
    return "BOL";
  }

  const quick = detectDocTypeByRegex(text);
  if (quick !== "UNKNOWN") {
    // Keep BOL strict to avoid swallowing PKL pages with "Bill of Lading No." field labels.
    if (quick === "BOL") {
      if (pklScore >= 6 && pklScore >= bolScore) {
        return "PKL";
      }
      if (bolScore >= 6) {
        return "BOL";
  ...
  if (looksLikePackingList(normalized, words)) {
    return "PKL";
  }
  if (looksLikeInvoice(normalized, words)) {
    return "INV";
  }
  if (looksLikeCertificate(normalized, words, "health")) {
    return "HC";
  }
  if (looksLikeCertificate(normalized, words, "phyto")) {
    return "PC";
  }
  if (looksLikeCOO(normalized, words)) {
    return "COO";
  }
  if (looksLikeCOA(normalized, words)) {
    return "COA";
  }
  if (looksLikeCAD(normalized, words)) {
    return "CAD";
  }
  if (
    hasApproxWord(words, "health", 2) &&
    hasApproxWord(words, "certificate", 3) &&
    !normalized.includes("analysis certificate") &&
    !normalized.includes("certificate of quantity and quality")
  ) {
    return "HC";
  }
  if (looksLikeInspectionCertificateUnknown(normalized)) {
    return "UNKNOWN";
  }
  if (looksLikePackingDetailUnknown(normalized)) {
    return "UNKNOWN";
  }
  if (looksLikeBOL(normalized, words)) {
    return "BOL";
  }
...
function getTypePrimaryScore(type, scores) {
  switch (type) {
    case "INV":
      return scores.invScore;
    case "PKL":
      return scores.pklScore;
    case "BOL":
      return scores.bolScore;
    case "PC":
      return scores.pcScore;
    case "HC":
      return scores.hcScore;
    case "COO":
      return scores.cooScore;
    case "COA":
      return scores.coaScore;
    case "CAD":
      return scores.cadScore;
    default:
      return 0;
  }
}
...
function resolveHeaderType(normalizedHeader, compactHeader, normalizedHeaderAscii) {
  const strictHeaderType = detectTypeByStrictHeader(
    normalizedHeader,
    compactHeader,
    normalizedHeaderAscii
  );
  if (strictHeaderType !== "UNKNOWN") {
    return strictHeaderType;
  }
...
function detectTypeByHeader(normalizedHeader) {
  ...
  if (
    normalizedHeader.includes("packing list") ||
    (normalizedHeader.includes("packing") && normalizedHeader.includes("list"))
  ) {
    return "PKL";
  }
  if (
    normalizedHeader.includes("phytosanitary") ||
    (normalizedHeader.includes("phyto") && normalizedHeader.includes("certificate")) ||
    (normalizedHeader.includes("plant") && normalizedHeader.includes("quarantine"))
  ) {
    return "PC";
  }
  if (normalizedHeader.includes("health certificate")) {
    return "HC";
  }
  if (normalizedHeader.includes("certificate of analysis")) {
    return "COA";
  }
  if (normalizedHeader.includes("or multimodal transport document")) {
    return "BOL";
  }
  if (
    normalizedHeader.includes("bill of lading") &&
    !normalizedHeader.includes("bill of lading no") &&
    !normalizedHeader.includes("shipping details")
  ) {
    return "BOL";
  }
  if (normalizedHeader.includes("certificate of origin")) {
    return "COO";
  }
  if (
    normalizedHeader.includes("commercial invoice") ||
    normalizedHeader.includes("tax invoice")
  ) {
    return "INV";
  }
  return "UNKNOWN";
...
function detectTypeByStrictHeader(normalizedHeader, compactHeader, normalizedHeaderAscii) {
  if (
    normalizedHeader.includes("commercial invoice") ||
    normalizedHeader.includes("tax invoice") ||
    compactHeader.includes("commercialinvoice")
  ) {
    return "INV";
  }
  if (
    normalizedHeader.includes("phytosanitary certificate") ||
    compactHeader.includes("phytosanitarycertificate") ||
    (normalizedHeader.includes("plant") && normalizedHeader.includes("protection")) ||
    normalizedHeaderAscii.includes("giay chung nhan kiem dich thuc vat")
  ) {
    return "PC";
  }
  if (
    normalizedHeader.includes("packing list") ||
    compactHeader.includes("packinglist")
  ) {
    return "PKL";
  }
  return "UNKNOWN";
...
function hasApproxWord(words, target, maxDistance) {
  ...
  const found = words.some((word) => {
    if (Math.abs(word.length - normalizedTarget.length) > maxDistance) {
      return false;
    }
...
function looksLikePackingList(normalized, words) {
  if (hasNearPhrase(normalized, "packing list")) {
    return true;
  }
...
function looksLikeInvoice(normalized, words) {
  if (hasNearPhrase(normalized, "commercial invoice") || hasNearPhrase(normalized, "tax invoice")) {
    return true;
  }
  if (
    normalized.includes("certificate of origin") ||
    normalized.includes("certificate of analysis") ||
    normalized.includes("certificate of quantity and quality") ||
    normalized.includes("analysis certificate") ||
...
function looksLikeInspectionCertificateUnknown(normalized) {
  const strongInspectionSignals =
    normalized.includes("certificate of quantity and quality") ||
    normalized.includes("analysis certificate") ||
    normalized.includes("results of inspection");
  const labSignals =
    normalized.includes("vinacontrol") ||
    normalized.includes("surveyor") ||
    normalized.includes("deputy manager") ||
    normalized.includes("fit for human consumption");
  const explicitKnownDocSignal =
    normalized.includes("health certificate") ||
    normalized.includes("certificate of origin") ||
    normalized.includes("certificate of analysis") ||
...
function looksLikePackingDetailUnknown(normalized) {
  const signals =
    (normalized.includes("detail of packing") &&
      normalized.includes("contract") &&
      normalized.includes("dop no")) ||
    (normalized.includes("detail of packing") && normalized.includes("batch number"));
  const explicitKnownDocSignal =
    normalized.includes("packing list") ||
    normalized.includes("commercial invoice") ||
    normalized.includes("certificate of origin") ||
...
function scoreHeaderProfiles(normalizedHeader, normalizedHeaderAscii) {
  ...
  govTokens.forEach((token) => {
    if (normalizedHeader.includes(token)) {
      government += 1;
    }
  });
  govAsciiTokens.forEach((token) => {
    if (normalizedHeaderAscii.includes(token)) {
      government += 1;
    }
  });
  carrierTokens.forEach((token) => {
    if (normalizedHeader.includes(token)) {
      carrier += 1;
    }
  });
  enterpriseTokens.forEach((token) => {
    if (normalizedHeader.includes(token)) {
      enterprise += 1;
    }
  });
...
function scoreINV(normalized, header, words) {
  let score = 0;
  if (header.includes("commercial invoice") || header.includes("tax invoice")) {
    score += 10;
  } else if (
    header.includes("invoice") &&
    !header.includes("invoice no") &&
    !header.includes("bill of lading") &&
    !header.includes("certificate")
  ) {
    score += 4;
  }
  if (normalized.includes("commercial invoice") || normalized.includes("tax invoice")) {
    score += 8;
  }
  if (hasApproxWord(words, "invoice", 2)) {
    score += 2;
  }
  if (words.includes("inv")) {
    score += 2;
  }
  if (normalized.includes("balance amount")) {
    score += 2;
  }
  if (normalized.includes("in words")) {
    score += 1;
  }
  if (
    normalized.includes("bill of lading") ||
...
function looksLikeBOL(normalized, words) {
  const hasCertificateSignals =
    normalized.includes("health certificate") ||
    normalized.includes("certificate of analysis") ||
    normalized.includes("certificate of origin") ||
    normalized.includes("certificate of admissibility") ||
    normalized.includes("phytosanitary");

  if (hasNearPhrase(normalized, "or multimodal transport document")) {
    return true;
  }
  if (hasNearPhrase(normalized, "bill of lading")) {
    if (normalized.includes("shipping details") || hasCertificateSignals) {
      return false;
    }
    return true;
  }
...
function scorePKL(normalized, header, words) {
  let score = 0;
  if (header.includes("packing list")) {
    score += 9;
  }
  if (hasNearPhrase(normalized, "packing list")) {
    score += 5;
  }
  if (hasApproxWord(words, "packing", 2)) {
    score += 2;
  }
  if (hasApproxWord(words, "list", 1)) {
    score += 1;
  }
  if (words.includes("pkl") || words.includes("pk1")) {
    score += 3;
  }
  if (normalized.includes("shipping details")) {
    score += 1;
  }
  if (
    normalized.includes("shipping details") &&
    (normalized.includes("consignor") || normalized.includes("consignee"))
  ) {
    score += 1;
  }
  if (normalized.includes("vendor fda") || normalized.includes("vendor duns")) {
    score += 1;
  }
  if (normalized.includes("container seal") || normalized.includes("container seal no")) {
    score += 2;
  }
  if (normalized.includes("gross weight") && normalized.includes("net weight")) {
    score += 3;
  }
  if (normalized.includes("marks") && normalized.includes("nos")) {
    score += 2;
  }
  if (normalized.includes("description of goods")) {
    score += 1;
  }
  if (
    normalized.includes("quantity") &&
    (normalized.includes("carton") || normalized.includes("ctn") || normalized.includes("package"))
  ) {
    score += 2;
  }
  if (
    ...
    hasApproxWord(words, "invoice", 2)
  ) {
    score -= 7;
  }
  if (normalized.includes("invoice no")) {
    score -= 2;
  }
  if (normalized.includes("balance amount")) {
    score -= 3;
...
function scoreBOL(normalized, header, words) {
  let score = 0;
  if (header.includes("or multimodal transport document")) {
    score += 11;
  } else if (
    header.includes("bill of lading") &&
    !header.includes("bill of lading no") &&
    !header.includes("shipping details")
  ) {
    score += 6;
  } else if (header.includes("bill of lading no")) {
    score += 1;
  }
  if (normalized.includes("or multimodal transport document")) {
    score += 4;
  }
  if (hasNearPhrase(normalized, "bill of lading")) {
    score += 2;
  ...
  // Penalize BOL if page has packing-list-style headline/content.
  if (header.includes("packing list")) {
    score -= 6;
  }
  if (hasNearPhrase(normalized, "packing list")) {
  ...
  if (normalized.includes("bill of lading no") && !header.includes("bill of lading")) {
    score -= 2;
  }
  if (header.includes("bill of lading no") && !header.includes("or multimodal transport document")) {
    score -= 3;
  }
  if (normalized.includes("shipping details") && !header.includes("bill of lading")) {
    score -= 8;
  }
...
function looksLikeCertificate(normalized, words, mode) {
  ...
  if (mode === "health") {
    return hasApproxWord(words, "health", 2) || words.includes("hc");
  }
  return (
    hasApproxWord(words, "phyto", 2) ||
    hasApproxWord(words, "phytosanitary", 3) ||
    hasApproxWord(words, "quarantine", 2) ||
    words.includes("pc")
  );
}
...
function scorePC(normalized, header, words) {
  let score = 0;
  if (header.includes("phytosanitary")) {
    score += 8;
  }
  if ((header.includes("phyto") && header.includes("certificate")) || header.includes("phytosanitary certificate")) {
    score += 6;
  }
  if (header.includes("plant") && header.includes("quarantine")) {
    score += 5;
  }
  if (normalized.includes("phytosanitary")) {
    score += 4;
  }
  if (normalized.includes("plant") && normalized.includes("quarantine")) {
    score += 4;
  }
  if (normalized.includes("additional declaration")) {
    score += 2;
  }
  if (hasApproxWord(words, "quarantine", 2)) {
    score += 2;
  }
  if (hasApproxWord(words, "phyto", 2) || hasApproxWord(words, "phytosanitary", 3)) {
    score += 2;
  }
  if (words.includes("pc")) {
    score += 1;
  }
...
function scoreHC(normalized, header, words) {
  let score = 0;
  if (header.includes("health certificate")) {
    score += 8;
  }
  if (normalized.includes("health certificate")) {
    score += 4;
  }
  if (hasApproxWord(words, "health", 2)) {
    score += 2;
  }
  if (hasApproxWord(words, "veterinary", 2)) {
    score += 2;
  }
  if (words.includes("hc")) {
    score += 1;
  }
  if (hasApproxWord(words, "health", 2) && hasApproxWord(words, "certificate", 3)) {
    score += 6;
  }
  if (normalized.includes("fit for human consumption")) {
    score += 3;
  }
  if (normalized.includes("vinacontrol")) {
    score += 1;
  }
  return score;
}
...
function scoreCOO(normalized, header, words) {
  let score = 0;
  if (header.includes("certificate of origin")) {
    score += 8;
  }
  if (normalized.includes("certificate of origin")) {
    score += 4;
  }
  if (hasApproxWord(words, "origin", 2)) {
    score += 2;
  }
  if (words.includes("coo")) {
    score += 1;
  }
  return score;
}
...
function scoreCOA(normalized, header, words) {
  let score = 0;
  if (header.includes("certificate of analysis")) {
    score += 8;
  }
  if (normalized.includes("certificate of analysis")) {
    score += 4;
  }
  if (hasApproxWord(words, "analysis", 2)) {
    score += 2;
  }
  if (words.includes("coa")) {
    score += 1;
  }
  return score;
}
...
function scoreCAD(normalized, header, words) {
  let score = 0;
  if (header.includes("certificate of admissibility")) {
    score += 8;
  }
  if (normalized.includes("certificate of admissibility")) {
    score += 4;
  }
  if (hasApproxWord(words, "admissibility", 3)) {
    score += 2;
  }
  if (words.includes("cad")) {
    score += 1;
  }
  return score;
}
...
function looksLikeCOA(normalized, words) {
  if (hasNearPhrase(normalized, "certificate of analysis")) {
    return true;
  }
  return (
    hasApproxWord(words, "certificate", 2) &&
    (hasApproxWord(words, "analysis", 2) || words.includes("coa"))
  );
}

function looksLikeCOO(normalized, words) {
  if (hasNearPhrase(normalized, "certificate of origin")) {
    return true;
  }
  return (
    hasApproxWord(words, "certificate", 2) &&
    (hasApproxWord(words, "origin", 2) || words.includes("coo"))
  );
}

function looksLikeCAD(normalized, words) {
  if (hasNearPhrase(normalized, "certificate of admissibility")) {
    return true;
  }
  return (
    hasApproxWord(words, "certificate", 2) &&
    (hasApproxWord(words, "admissibility", 3) || words.includes("cad"))
  );
}
...
function buildGroups(pageTypes) {
  ...
  for (let i = 0; i < raw.length; i += 1) {
    ...
    // If UNKNOWN is between same-type groups, bridge them.
    ...
    // Keep UNKNOWN separate unless it can safely bridge same-type neighbors.
...
async function classifyPageByBackendHeaderAI(page, pageNumber, hint) {
  ...
  let imageDataUrl;
  try {
    imageDataUrl = await buildHeaderImageDataUrl(page);
  } catch {
    return null;
  }

  const cacheKey = fastHash(imageDataUrl.slice(0, 18000));
  if (AI_HEADER_DECISION_CACHE.has(cacheKey)) {
    return AI_HEADER_DECISION_CACHE.get(cacheKey);
  }
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (type !== "UNKNOWN" && confidence >= best.confidence) {
        best = { type, confidence };
      }
      if (best.type !== "UNKNOWN" && best.confidence >= AI_MIN_CONFIDENCE) {
        break;
      }
    } catch {
  ...
  const decision = {
    type: accepted ? best.type : "UNKNOWN",
    confidence: best.confidence,
    accepted
  };
  AI_HEADER_DECISION_CACHE.set(cacheKey, decision);
  if (AI_HEADER_DECISION_CACHE.size > 300) {
    AI_HEADER_DECISION_CACHE.clear();
  }
...
async function runOcrOnPage(page, pageNumber, options) {
  ...
  if (preprocess) {
    applyThreshold(context, canvas.width, canvas.height);
  }
...
async function extractPageTypes(pdf) {
  ...
  for (let i = 1; i <= pdf.numPages; i += 1) {
    ...
    if (nativeText.length < MIN_TEXT_CHARS) {
      source = "ocr-title";
      const titleText = await runOcrOnPage(page, i, {
        scale: OCR_TITLE_SCALE,
        preprocess: false,
        headerOnly: true,
        cropRatio: OCR_TITLE_HEIGHT_RATIO,
        stage: "title"
      });
      ...
      if (typeFirstPass === "UNKNOWN") {
        source = "ocr-header";
        const headerOcr = await runOcrOnPage(page, i, {
          scale: OCR_HEADER_SCALE,
          preprocess: true,
          headerOnly: true,
          stage: "header"
        });
      ...
      if (typeFirstPass === "UNKNOWN") {
        if (USE_BACKEND_ANALYZER) {
          log(`Page ${i}: AI header classify...`);
        }
        const aiDecision = await classifyPageByBackendHeaderAI(page, i, headerText);
        const aiAccepted =
          aiDecision?.accepted &&
          hasReliableRuleSupportForType(aiDecision.type, finalText, headerText);
        if (aiAccepted) {
          aiForcedType = aiDecision.type;
          aiConfidence = aiDecision.confidence;
          source = `ai-header(${aiConfidence.toFixed(2)})`;
        } else {
          if (aiDecision?.accepted) {
            log(`Page ${i}: AI decision rejected by rule support gate.`);
          }
          source = "ocr-full";
          const fullText = await runOcrOnPage(page, i, {
      ...
      if (typeFirstPass === "UNKNOWN" && aiForcedType === "UNKNOWN") {
        source = "ocr-retry";
        const retryText = await runOcrOnPage(page, i, {
      ...
      // If text-layer exists but classification is UNKNOWN, OCR title first, then AI/OCR recovery.
      let typeFromTextLayer = detectDocType(finalText, headerText);
      if (typeFromTextLayer === "UNKNOWN") {
        source = "text-layer+ocr-title";
        const titleRecover = await runOcrOnPage(page, i, {
          scale: OCR_TITLE_SCALE,
          preprocess: false,
          headerOnly: true,
          cropRatio: OCR_TITLE_HEIGHT_RATIO,
      ...
      if (typeFromTextLayer === "UNKNOWN") {
        ...
        if (aiAccepted) {
          aiForcedType = aiDecision.type;
          aiConfidence = aiDecision.confidence;
          source = `text-layer+ai-header(${aiConfidence.toFixed(2)})`;
        } else {
          if (aiDecision?.accepted) {
            log(`Page ${i}: AI decision rejected by rule support gate.`);
          }
          ...
          if (typeFromTextLayer === "UNKNOWN") {
            ...
            const fullRecover = await runOcrOnPage(page, i, {
              scale: OCR_RENDER_SCALE,
              preprocess: false,
              headerOnly: false,
              stage: "full-recover"
    ...
    const unknownBoost =
      (isInspectionUnknown ? 6 : 0) +
      (isPackingDetailUnknown ? 5 : 0) +
      (type === "UNKNOWN" ? 1 : 0);
    const scores = buildTypeScores(normalized, normalizedHeader, normalizedHeaderAscii, words);
    const headerType = resolveHeaderType(
      normalizedHeader,
      normalizedHeader.replace(/\s+/g, ""),
      normalizedHeaderAscii
    );
...
function transitionPenalty(prevType, nextType) {
  ...
  if (prevType === "UNKNOWN" || nextType === "UNKNOWN") {
    return -0.5;
  }
...
function buildStateScoreMap(item) {
  ...
  if (item.headerType && item.headerType !== "UNKNOWN") {
    map[item.headerType] += 5;
  }
  ...
  if (item.unknownBoost) {
    ...
    if (item.unknownBoost >= 5) {
      KNOWN_DOC_TYPES.forEach((type) => {
        map[type] -= 1.5;
      });
    }
  }
...
function refinePageTypes(pageTypes) {
  ...
  const dp = Array.from({ length: pageTypes.length }, () =>
    Array(stateCount).fill(Number.NEGATIVE_INFINITY)
  );
  const backtrack = Array.from({ length: pageTypes.length }, () => Array(stateCount).fill(-1));

  for (let s = 0; s < stateCount; s += 1) {
    const state = CONTEXT_STATES[s];
    dp[0][s] = scoreMaps[0][state];
  }
  ...
  for (let s = 1; s < stateCount; s += 1) {
    if (dp[pageTypes.length - 1][s] > dp[pageTypes.length - 1][bestStateIdx]) {
      bestStateIdx = s;
    }
  }
  ...
  for (let i = pageTypes.length - 1; i > 0; i -= 1) {
    bestPath[i - 1] = backtrack[i][bestPath[i]];
  }
  ...
  // Recovery heuristic: scanned HC continuation page can lose header OCR signal.
  for (let i = 1; i < refined.length; i += 1) {
    ...
    if (
      ...
      prev?.type === "HC" &&
      curr.chars >= 1200 &&
      String(curr.source || "").includes("ocr")
    ) {
      curr.type = "HC";
      curr.primaryScore = getTypePrimaryScore(curr.type, curr);
    }
  }
...
function buildPreviewUrlAtTop(previewUrl) {
  const base = String(previewUrl || "");
  if (!base) {
    return "";
  }
  // Force built-in PDF viewer to open from first page at top.
  return `${base}#page=1&view=FitH&zoom=page-width`;
}
...
function openPreview(docId) {
  ...
  previewFrame.removeAttribute("src");
  previewFrame.src = buildPreviewUrlAtTop(doc.previewUrl);
  previewModal.hidden = false;
  previewFrame.onload = () => {
    try {
      previewFrame.contentWindow?.scrollTo(0, 0);
    } catch (_error) {
      // Ignore cross-context scroll access issues.
    }
  };
}
...
function createDocCard(doc) {
  ...
  card.innerHTML = `
    <div class="doc-select-col">
      <input class="doc-select" type="checkbox" data-doc-id="${doc.id}" checked aria-label="Select ${doc.fileName}">
    </div>
...
function getDocById(docId) {
  return splitDocs.find((doc) => doc.id === docId);
}
...
function syncSelectAllState() {
  ...
  const allChecks = Array.from(resultsGrid.querySelectorAll(".doc-select"));
  ...
  selectAllDocs.checked = checkedCount === allChecks.length;
  selectAllDocs.indeterminate = checkedCount > 0 && checkedCount < allChecks.length;
}

function setAllDocsSelected(nextChecked) {
  const allChecks = Array.from(resultsGrid.querySelectorAll(".doc-select"));
...
async function handleProcess() {
  ...
  if (!vendorCode || !poCode) {
    log("Please input both Vendor code and PO#.");
    return;
  }
  ...
  try {
    ...
    log("OCR fallback: ON");
    log(
      `AI header classifier: ${USE_BACKEND_ANALYZER ? "ON" : "OFF"} (threshold=${AI_MIN_CONFIDENCE}, url=${BACKEND_ANALYZER_URL || "N/A"})`
    );
    log("Detecting doc type per page...");
    const pageTypes = await extractPageTypes(pdf);
    pageTypes.forEach((item) =>
      log(
        `Page ${item.pageNumber}: ${item.type} (source=${item.source}, ai=${item.aiType || "UNKNOWN"}:${(item.aiConfidence || 0).toFixed(2)}, chars=${item.chars}, score=${item.primaryScore}, header=${item.headerType || "UNKNOWN"}, inv=${item.invScore}, pkl=${item.pklScore}, bol=${item.bolScore}, pc=${item.pcScore}, hc=${item.hcScore}, coo=${item.cooScore}, coa=${item.coaScore}, cad=${item.cadScore})`
      )
    );

    const groups = buildGroups(pageTypes);
    groups.forEach((group, idx) =>
      log(`Group ${idx + 1}: ${group.type}, pages: ${group.pages.join(", ")}`)
    );
    ...
    processBtn.textContent = "Split Documents";
  }
}

async function handleDownloadAll() {
  if (!splitDocs.length) {
    log("No split files yet.");
    return;
  }
...
dropZone.addEventListener("drop", (e) => {
  ...
  if (file && file.type === "application/pdf") {
    ...
    fileNameEl.textContent = "PDF file only.";
  }
});
...
resultsGrid.addEventListener("click", (event) => {
  ...
  if (previewTarget) {
    openPreview(previewTarget.getAttribute("data-preview-id"));
    return;
  }
...
resultsGrid.addEventListener("change", (event) => {
  if (event.target.closest(".doc-select")) {
    syncSelectAllState();
  }
});

selectAllDocs?.addEventListener("change", (event) => {
  setAllDocsSelected(event.target.checked);
});
